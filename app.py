...
if st.session_state.page == 1:
    ...
    with st.form("part1"):
        ...
        #  Growth Rates
        st.markdown("**/Web Traffic Growth**")
        _cols = st.columns(5)
        with _cols[0]:
...
df = pd.DataFrame({
    ...
    "Cross-Over Month Trial-To-Paid":free_trial_days / df["Days Count"],
    "Trial-To-Paid Within Month": 1 - df["Cross-Over Month Trial-To-Paid"]
...
for i in range(1, 60):
    afmar_growth_rate = get_afmar_growth_rate(i)  # ✅ Store growth rate in a different variable
...
# Group by Year and sum numeric values
df_financials_by_year = df_financials.groupby("Year", as_index=False).sum(numeric_only=True)
...
# Ensure the Month column is sorted properly
